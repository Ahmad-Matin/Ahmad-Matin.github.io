...
function showCartItems() {
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-center">
        <div class="d-flex flex-row row align-items-center p-0 my-2 mx-4 w-100 justify-content-between">
            <h5 class="checkout card-title col-sm-12 col-md-8 p-0 my-2 text-left">${cartItems[i].name}</h5>
            <h5 class="checkout card-title col-sm-12 col-md-4 p-0 my-2 text-right item-price">$${cartItems[i].price}</h5>
        </div>
...
function updateCartTotal() {
    ...
    for (let i = 0; i < itemsInCart.length; i++) {
        const itemPriceElement = itemsInCart[i].children[0].children[0].children[1].innerText.replace('$', '');
        const itemPrice = parseFloat(itemPriceElement);
        const itemQuantityElement = (itemsInCart[i].children[0].children[1].children[1].innerText);
...
function addItemsToCart(product) {
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between">
        <div class="d-flex flex-row row align-items-center p-0 my-2 mx-4 w-100 justify-content-between">
           <h5 class="checkout card-title col-sm-12 col-md-4 p-0 my-2 text-left">${product.name}</h5>
           <h5 class="checkout card-title col-sm-12 col-md-4 p-0 my-2 text-right item-price">$${product.price}</h5>
        </div>
        <div class="d-flex col p-0 my-2 mx-4 justify-content-between align-items-center cart-quantity-container">
            <button class="btn btn-warning px-3 remove-button rounded-0">-</button>
...
function createCartItem(productId, productName, productType, productPrice) {
    return {
        id: productId,
        name: productName,
        type: productType,
...
//    let productImg = e.target.parentElement.parentElement.parentElement.parentElement.children[0].children[0].attributes.src.value;
    ...
    let itemNames = [];
    for (let i = 0; i < itemsInCart.length; i++) {
        let itemName = itemsInCart[i].children[0].children[0].children[0].innerText;
    ...
    if (itemNames.includes(productName)) {
        ...
        let itemQuantityElement = itemsInCart[itemCartIndex].children[0].children[1].children[1];
        ...
        const newCartItem = createCartItem(productId, productName, productType, productPrice);
...
let cartScrollSection = document.getElementById("cart-section")
  cartScrollSection.scrollTop = cartScrollSection.scrollHeight - cartScrollSection.clientHeight;
...
document.querySelector("#checkout-container").addEventListener('click', (e) => {
    ...
    if (e.target.classList.contains("add-button")) {
        for (let i = 0; i < cartItems.length; i++) {
            let productName = e.target.parentElement.parentElement.children[0].children[0].innerText;
            if (productName == cartItems[i].name) {
